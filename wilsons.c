...
{
    vec2 returnValue = {a.x + b.x, a.y + b.y};
    return returnValue;
}

vec2 addScaler(vec2 a, int b)
{
    vec2 returnValue = {a.x + b, a.y + b};
    return returnValue;
}

vec2 scale(vec2 a, double b)
{
    vec2 returnValue = {int(a.x * b), int(a.y * b)};
    return returnValue;
}
...
const rgb24 COLOR_UNUSED_CELLS    = {};
...
void setPixelAtCoords(rendererProps* renderer, vec2 coords, uint8_t r, uint8_t g, uint8_t b)
{
    rgb24* pixel = &renderer->frameBuffer.pixels[coords.x + coords.y * renderer->screenSize.x];
    setPixel(pixel, r, g, b);
}

void setPixelAtXY(rendererProps* renderer, int x, int y, uint8_t r, uint8_t g, uint8_t b)
{
    vec2 coords = {x, y};
    setPixelAtCoords(renderer, coords, r, g, b);
}

void setPixelAtCoordsColor(rendererProps* renderer, vec2 coords, rgb24 pixel)
{
    setPixelAtCoords(renderer, coords, pixel.r, pixel.g, pixel.b);
}
...
void testFrameRender(rendererProps* renderer)
{
    ...
    { 
        for(int x = 0; x < renderer->screenSize.x; x++)
        {
            for(int y = 0; y < renderer->screenSize.y; y++)
            {
                setPixelAtXY(renderer, x, y, (uint8_t)(x % 256), uint8_t(y % 256), uint8_t(frame % 256));
            }
        }
...
void drawFill(rendererProps* renderer, rgb24 color)
{
    for(int y = 0; y < renderer->screenSize.x; y++)
    {
        ...
        {
            vec2 pos = {x, y};
            setPixelAtCoordsColor(renderer, pos, color);
        }
    }
}
...
void drawLine(rendererProps* renderer, vec2 pos1, vec2 pos2, rgb24 color)
{
    ...
    if(abs(dx) >= abs(dy) && dx != 0)
    {
        ...
        {
            y = m * (x - pos1.x) + pos1.y;
            vec2 coords = {(int)x, (int)round(y)};
            if(checkBounds(renderer, coords))
                setPixelAtCoordsColor(renderer, coords, color);
        }
    } else if(dy != 0)
    {
        // using y coord
    ...
    } else
    {
        ...
        setPixelAtCoordsColor(renderer, pos1, color);
    }
}
...
void testDraws(rendererProps* renderer)
{
    ...
    for(int frame = 0; frame < frames; frame++)
    {
        drawFill(renderer, COLOR_BLACK);

        vec2 circleCenter = {50 + (int)(50. * cos(frame * 2. * M_PI / 120.)), 50 + (int)(50. * sin(frame * 2. * M_PI / 120.))};
        drawCircle(renderer, circleCenter, 50, COLOR_BLUE);

        vec2 boxPos = {frame - 25, 5};
        vec2 boxSize = {50, frame+50};
        drawBox(renderer, boxPos, boxSize, COLOR_WHITE);

        vec2 boxPos2 = {50, 75 - frame};
        vec2 boxSize2 = {50, 50};
        drawBox(renderer, boxPos2, boxSize2, COLOR_WHITE);

        vec2 circlePos = {50 + (int)(10. * cos(frame * 2. * M_PI / 30.)), 50 + (int)(10. * sin(frame * 2. * M_PI / 30.))};
        drawCircle(renderer, circlePos, 25, COLOR_RED);

        vec2 linePos1 = {50 + (int)(30. * cos(frame * 2. * M_PI / 60.)), 50 + (int)(30. * sin(frame * 2. * M_PI / 60.))};
        vec2 linePos2 = {50 + (int)(-100. * cos(frame * 2. * M_PI / 60.)), 50 + (int)(-100. * sin(frame * 2. * M_PI / 60.))};
        drawLine(renderer, linePos1, linePos2, COLOR_GREEN);
...
void renderLinkedList(rendererProps* renderer, maze_renderer* mazeRenderer, doubly_linked_list* linkedList, rgb24 cellColor, rgb24 lineColor)
{
    ...
    do
    {
        ...
        vec2 size = {mazeRenderer->cellSize, mazeRenderer->cellSize};
        drawBox(renderer, pos, size, cellColor);
...
void renderMaze(rendererProps* renderer, maze_t* maze)
{
    ...
    rgb24 unusedCellColor = {50, 50, 50};
    rgb24 unusedLineColor = {50, 0, 0};
    renderLinkedList(renderer, &mazeRenderer, &maze->unused, unusedCellColor, unusedLineColor);
    rgb24 randomWalkCellColor = {30, 30, 200};
    rgb24 randomWalkLineColor = {230, 230, 230};
    renderLinkedList(renderer, &mazeRenderer, &maze->randomWalk, randomWalkCellColor, randomWalkLineColor);
